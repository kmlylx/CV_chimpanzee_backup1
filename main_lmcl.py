...
#                opt.weight_decay, opt.batch_size, opt.temp, opt.trial)
    opt.model_name = '{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}_size_{}_pretrained_{}_scale0.8'.\
        format(opt.method, opt.dataset, opt.model, opt.learning_rate,
               opt.weight_decay, opt.batch_size, opt.temp, opt.trial, opt.size, opt.pretrained)
...
def set_loader(opt):
    ...
    if opt.dataset == 'cifar10':
        ...
        train_dataset = datasets.ImageFolder(root=opt.data_folder+"/train",
                                            transform=train_transform)
        train_dataset = MyImageFolder(root=opt.data_folder+"/train",
                                            transform=train_transform)
        print(train_dataset.class_to_idx)
    
#         val_dataset = datasets.ImageFolder(root=opt.data_folder+"/val",
#                                             transform=val_transform)
        val_dataset = MyImageFolder(root=opt.data_folder+"/val",
                                            transform=val_transform)
...
#     train_loader = set_loader(opt)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if epoch % opt.eval_freq == 0:
            ...
            if val_acc > best_acc:
                best_acc = val_acc
            with open('/code/validate.txt', 'a') as f:
